...
def listener():
	...
	subprocess.call(["./seabreeze_test_posix","400",str(sampleNum), str(tStruct.tm_year), str(tStruct.tm_mon), str(tStruct.tm_mday), str(tStruct.tm_hour), str(tStruct.tm_min), str(tStruct.tm_sec), str(alt), str(roll), str(pitch), str(yaw), str(lat), str(lon),filename]);
	#sleep(4)
	#p.kill()
...
def isArmed_callback():
	...
	subprocess.call(["./seabreeze_test_posix","0",filename]);
