...
int main(int argc, char **argv) {
    ...
    seabreeze_get_spectrometer_type(spec_index, &error, type, sizeof(type));
    ...
    for (unsigned i = 0; i < iterations; i++) {
        ...
        seabreeze_set_integration_time(spec_index, &error, integ1 * MS_TO_USEC);
        ...
        seabreeze_set_integration_time(spec_index, &error, integ2 * MS_TO_USEC);
        check_error(i, &error, "setIntegrationTime(A)");
