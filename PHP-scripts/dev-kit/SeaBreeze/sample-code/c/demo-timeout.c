...
void initialize_spectrometer(int index, unsigned long integ_time_millis, int trigger_mode) {
    ...
    seabreeze_set_integration_time(index, &error, integ_time_millis * 1000);
    check_error(index, &error, "seabreeze_set_integration_time");
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < device_count; i++) {
        ...
        if (check_error(i, &error, "seabreeze_get_spectrometer_type")) 
            continue;
